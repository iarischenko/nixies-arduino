...
void PD_UFP_core_c(struct PD_UFP_core_c *context)
{
    ...
    context->status_power = STATUS_POWER_NA;
}

void PD_UFP_core_c_init(enum PD_power_option_t power_option)
{
    PD_UFP_core_c_init_PPS(0, 0, power_option);
}

void PD_UFP_core_c_init_PPS(struct PD_UFP_core_c *context, uint16_t PPS_voltage,
        uint8_t PPS_current, enum PD_power_option_t power_option)
{
    /* Initialize FUSB302 */
    /* Set FUSB302 int pin input ant pull up */
    context->FUSB302.i2c_address = 0x22;
    context->FUSB302.i2c_read = FUSB302_i2c_read;
    context->FUSB302.i2c_write = FUSB302_i2c_write;
    context->FUSB302.delay_ms = FUSB302_delay_ms;

    if (FUSB302_init(&FUSB302) == FUSB302_SUCCESS
        && FUSB302_get_ID(&FUSB302, 0, 0) == FUSB302_SUCCESS) {
        context->status_initialized = 1;
    }
    ...
    PD_protocol_init(&context->protocol);
    PD_protocol_set_power_option(&context->protocol, power_option);
    PD_protocol_set_PPS(&context->protocol, PPS_voltage, PPS_current, false);
...
void PD_UFP_core_c_run(void)
{
    ...
    if (timer() || digitalRead(PIN_FUSB302_INT) == 0) {
        ...
        for (uint8_t i = 0; i < 3 &&
                FUSB302_alert(&FUSB302, &FUSB302_events) != FUSB302_SUCCESS; i++) {}
        if (FUSB302_events) {
        ...
        uint8_t PPS_current)
{
    if (status_power == STATUS_POWER_PPS &&
            PD_protocol_set_PPS(&context->protocol, PPS_voltage, PPS_current,
                true)) {
        send_request = 1;
        return true;
    }
...
{
    if (PD_protocol_set_power_option(&context->protocol, power_option)) {
        send_request = 1;
    }
}
...
{
	i2c_transfer7(I2C1, dev_addr, &reg_addr, 1, data, count);
...
static FUSB302_ret_t FUSB302_i2c_write(uint8_t dev_addr, uint8_t reg_addr,
        uint8_t *data, uint8_t count)
{
	i2c_transfer7(I2C1, dev_addr, &reg_addr, count, data, 0);
    return FUSB302_SUCCESS;
}
